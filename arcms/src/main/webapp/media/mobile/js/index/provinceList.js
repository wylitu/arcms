...
function getAllProvince(){
    ...
    for(var i = 0; i < provinceList.length; i++){
        provinces.push(provinceList[i].name)
    }
    return provinces
}
function getCityByProvince(province){
    for(var i = 0; i < provinceList.length; i++){
        var innerProvince = provinceList[i]
        if(innerProvince.name == province){
            var cities = []
            for(var j = 0 ; j < innerProvince.cityList.length; j++){
                cities.push(innerProvince.cityList[j].name)
            }
            return cities
        }
    }
...
function getAreaByCity(city){
    var lastChar = city.substr(city.length -1)
    if(lastChar != '市'){
        city = city + '市'
    }
    for(var i = 0; i < provinceList.length; i++){
        ...
        for (var j = 0; j < province.cityList.length; j++){
            var innerCity = province.cityList[j]
            if(city == innerCity.name){
                return innerCity.areaList
            }
        }
    }
}