(function(){
    // 大陆省份
    var provinces_land = ["北京市", "天津市", "上海市", "重庆市", "河北省", "山西省", "内蒙古", "辽宁省", "吉林省", "黑龙江省", "江苏省", "浙江省", "安徽省", "福建省", "江西省", "山东省", "河南省", "湖北省", "湖南省", "广东省", "广西", "海南省", "四川省", "贵州省", "云南省", "西藏", "陕西省", "甘肃省", "青海省", "宁夏", "新疆"];
    // 整个中国
    var provinces_china = ["北京市", "天津市", "上海市", "重庆市", "河北省", "山西省", "内蒙古", "辽宁省", "吉林省", "黑龙江省", "江苏省", "浙江省", "安徽省", "福建省", "江西省", "山东省", "河南省", "湖北省", "湖南省", "广东省", "广西", "海南省", "四川省", "贵州省", "云南省", "西藏", "陕西省", "甘肃省", "青海省", "宁夏", "新疆", "香港", "澳门", "台湾省"];
    
    window.Dsy = Dsy;
    function Dsy(options) {
        var options = options || {};
        var self = this;
        var isMainLand = options.isMainLand?true:false;// 是否大陆内地省份 排除港澳台

        this.opts = {};
        // select选择器
        var s = this.s = options.s || ["s1", "s2", "s3"];
        var p_key = options.p_key || '';
        var c_key = options.c_key || '';
        var t_key = options.t_key || '';
        // select选择器第一级option
        this.opt0 = options.opt0 || ["省", "市", "区/县"];
        // 设置省份
        if(isMainLand){
            this.provinces = provinces_land;
            
        }else {
            this.provinces = provinces_china;
        }

        if(options.provinces) {
            this.provinces = options.provinces;
        }
        Dsy.update("0",this.provinces);
        
        for (var i = 0; i < s.length - 1; i++){
            document.getElementById(s[i]).onchange = (function(i){
                return function(){
                    self.change(i+1);
                }
            })(i);
        ...
        this.preselect(p_key,c_key,t_key);

    }
    Dsy.add = function (id, iArray) {
        Dsy.Items[id] = iArray;
    }
    Dsy.update = function(id,iArray){
        Dsy.Items[id] = iArray;
    }
    Dsy.Exists = function (id) {
        if (typeof(Dsy.Items[id]) == "undefined") return false;
        return true;
    }
    /* 初始化第一个省份 p_key: 浙江省 or 山西省 */
    Dsy.prototype.preselect = function(p_key,c_key,t_key){
        var index,cIndex,tIndex;
        ...
        if(c_key){
            var cityArr = Dsy.Items['0_'+index];
            ...
            document.getElementById(s[1]).selectedIndex = cIndex +1;
            this.change(2);
        }

        if(t_key){
            var townArr = Dsy.Items['0_'+index+'_'+cIndex];
           for (j = 0; j < townArr.length; j++) {
                if (t_key == townArr[j]) {
                    tIndex = j;
                    break;
                }
            }
            document.getElementById(s[2]).selectedIndex = tIndex +1;
        }
       
    };
    /* v: 0,1,2 */
    Dsy.prototype.change = function(v){
        var str = "0";
        var s = this.s;
        var self = this;
        var opt0 = this.opt0;
